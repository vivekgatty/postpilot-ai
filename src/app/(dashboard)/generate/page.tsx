...
import { NICHE_OPTIONS, SYSTEM_TONES, SYSTEM_FORMATS } from '@/lib/constants'
import type {
  NicheType, GeneratedPost, UsageInfo, Profile,
  CustomTone, CustomFormat, PlanType, ToneType,
} from '@/types'
...
function SkeletonCard({ delay }: { delay: number }) {
  return (
    <div className="bg-white rounded-2xl border border-[#E5E4E0] p-5 overflow-hidden" style={{ animationDelay: `${delay}ms` }}>
...
function GeneratePageInner() {
  ...
  const [customTonePrompt, setCustomTonePrompt]  = useState<string | undefined>()
  const [formatId, setFormatId]                 = useState('listicle')
  const [formatIsCustom, setFormatIsCustom]     = useState(false)
  const [customFormatPrompt, setCustomFormatPrompt] = useState<string | undefined>()
  ...
  useEffect(() => {
    ...
    fetch('/api/custom-tones')
      .then(r => r.json())
      .then((d: { tones?: CustomTone[] }) => setCustomTones(d.tones ?? []))
      .catch(() => {})

    fetch('/api/custom-formats')
      .then(r => r.json())
      .then((d: { formats?: CustomFormat[] }) => setCustomFormats(d.formats ?? []))
      .catch(() => {})
  }, [refreshUsage])

  // Refresh helpers after create/update/delete
  const refreshCustomTones = useCallback((tone: CustomTone) => {
    if (!tone.is_active) {
      setCustomTones(prev => prev.filter(t => t.id !== tone.id))
    } else {
      setCustomTones(prev => {
        const idx = prev.findIndex(t => t.id === tone.id)
        return idx >= 0 ? prev.map(t => t.id === tone.id ? tone : t) : [tone, ...prev]
      })
    }
  }, [])

  const refreshCustomFormats = useCallback((fmt: CustomFormat) => {
    if (!fmt.is_active) {
      setCustomFormats(prev => prev.filter(f => f.id !== fmt.id))
    } else {
      setCustomFormats(prev => {
        const idx = prev.findIndex(f => f.id === fmt.id)
        return idx >= 0 ? prev.map(f => f.id === fmt.id ? fmt : f) : [fmt, ...prev]
      })
    }
  }, [])
    ...
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          ...
          niche,
          variations:         3,
          customTonePrompt:   toneIsCustom ? customTonePrompt : undefined,
  ...
  const handleSave = useCallback(async (post: GeneratedPost) => {
    ...
    const safeTone: ToneType = Object.keys(SYSTEM_TONES).includes(toneId)
      ? toneId as ToneType
      : 'professional'
    try {
      ...
      startTransition(() => setSavedSet(prev => new Set(Array.from(prev).concat(post.variation))))
  ...
  }, [savingIdx, toneId, niche, topic, startTransition, toast])
  ...
  // ── Label helpers for result chips ──────────────────────────────────────
  const toneLabel  = toneIsCustom
    ? (customTones.find(t => t.id === toneId)?.name ?? 'Custom')
    : (SYSTEM_TONES[toneId]?.label ?? toneId)

  const formatLabel = formatIsCustom
    ? (customFormats.find(f => f.id === formatId)?.name ?? 'Custom')
    : (SYSTEM_FORMATS[formatId]?.label ?? formatId)

  const toneColor   = !toneIsCustom ? (SYSTEM_TONES[toneId]?.colorClass ?? '#1D9E75') : '#1D9E75'
  const safeToneForCard: ToneType = Object.keys(SYSTEM_TONES).includes(toneId) ? toneId as ToneType : 'professional'
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 pb-20">
      ...
      {/* ─── LEFT PANEL ──────────────────────────────────────────────────── */}
      <div className="w-full lg:w-[420px] lg:flex-shrink-0 space-y-5">
        ...
        <div>
          ...
          <p className={cn(
            ...
            wordCount === 0 ? 'text-gray-400' : wordCount < 50 ? 'text-amber-500' : 'text-[#1D9E75]',
          )}>
            {wordCountLabel}
          </p>
        </div>
        ...
        <div>
          ...
          <ToneSelector
            value={toneId}
            onChange={(id, isCustom, prompt) => { setToneId(id); setToneIsCustom(isCustom); setCustomTonePrompt(prompt) }}
            disabled={generating}
            userPlan={userPlan}
            customTones={customTones}
            onCustomToneCreated={refreshCustomTones}
          />
        </div>

        {/* Format selector */}
        <div>
          <label className="block text-sm font-semibold text-[#0A2540] mb-2">Post format</label>
          <FormatSelector
            value={formatId}
            onChange={(id, isCustom, prompt) => { setFormatId(id); setFormatIsCustom(isCustom); setCustomFormatPrompt(prompt) }}
            disabled={generating}
            userPlan={userPlan}
            customFormats={customFormats}
            onCustomFormatCreated={refreshCustomFormats}
          />
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">Helps tailor examples and context to your audience</p>
        </div>

        {/* Usage meter */}
        {usage && usage.plan === 'free' && usage.limit !== -1 && (
          <div className="bg-gray-50 rounded-xl px-4 py-3.5 border border-[#E5E4E0] space-y-2">
            <p className="text-xs font-semibold text-gray-600">{usage.used} of {usage.limit} free generations used</p>
            <UsageMeter used={usage.used} plan={usage.plan} resetDate={usage.resetDate} variant="bar" />
          </div>
        )}

        {/* Generate CTA */}
        {atLimit ? (
          <a href="/settings" className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gradient-to-r from-purple-600 to-[#1D9E75] text-white text-sm font-bold shadow hover:opacity-90 transition-opacity">
          ...
          <button type="button" onClick={handleGenerate} disabled={!canGenerate}
            className={cn(
              ...
              canGenerate ? 'bg-[#1D9E75] hover:bg-[#178a64] text-white shadow-sm hover:shadow' : 'bg-gray-100 text-gray-400 cursor-not-allowed',
            )}
          >
            {generating
              ? <><Loader2 className="w-4 h-4 animate-spin flex-shrink-0" /><span key={msgIdx}>{LOADING_MESSAGES[msgIdx]}</span></>
              : <><Sparkles className="w-4 h-4" />Generate 3 posts</>}
          </button>
        )}
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="text-sm font-semibold text-[#0A2540]">{posts.length} variations generated</p>
                <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">
                  <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: toneColor }} />
                  {toneLabel} tone
                </span>
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#E1F5EE] text-[#1D9E75]">
                  {formatLabel} format
                </span>
              </div>
